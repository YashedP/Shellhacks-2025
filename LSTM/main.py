...
files = [os.path.join("./output_segments", f) for f in os.listdir("./output_segments") if f.endswith(".gpkg")]
...
if __name__ == "__main__":
    test()
    if files:
        # TODO: Understanding: df has timestamp, segment, geometry which has multiple LineStrings which has coordinate points which then has (lat, long, elevation), disregard the elevation when training
        ...
        for i, file_path in enumerate(files):
            ...
            final_loss = train_model_on_segment(model, optimizer, criterion, X_tensor, y_tensor, epochs=EPOCHS)
            ...
            if i == 0:
                ...
                X_test, _ = prepare_lstm_sequences_single_segment(test_segment, sequence_length=SEQUENCE_LENGTH)
                if len(X_test) > 0:
                    forecast_steps = FORECAST_STEPS
                    predictions = forecast(model, torch.FloatTensor(X_test[0]), forecast_steps)
