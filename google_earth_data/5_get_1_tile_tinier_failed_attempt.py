...
EXPORT_SCALE = 30
TILE_SCALE_METERS = 25000   # 25 km × 25 km tiles
DEBUG_ONE_TILE = True
MAX_WORKERS = 4
...
# ================================
# GRID CREATION (true metric squares)
# ================================
def make_grid(region, scale=TILE_SCALE_METERS):
    # Use UTM zone 17N (Florida, meters)
    proj = ee.Projection("EPSG:32617").atScale(scale)

    lonlat = ee.Image.pixelLonLat().reproject(proj)
    x = lonlat.select("longitude").divide(scale).floor()
    y = lonlat.select("latitude").divide(scale).floor()
    ...
    grid = grid_id.reduceToVectors(
        ...
        crs="EPSG:32617"   # ensures tiles are true metric squares
    )
    return grid
...
def download_tile(img, geom, out_file, scale=EXPORT_SCALE):
    try:
        url = img.getDownloadURL({
            "scale": scale,
            "crs": "EPSG:32617",       # export in UTM zone 17N (meters)
            "region": geom.getInfo()
        })
...
if DEBUG_ONE_TILE:
    ...
    # Landsat collections (Surface Reflectance)
